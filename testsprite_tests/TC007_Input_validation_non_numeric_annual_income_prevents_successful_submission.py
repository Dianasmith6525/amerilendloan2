...
async def run_test():
    ...
    try:
        ...
        # -> Wait 15 seconds for React to mount, then navigate to /check-offers
        await page.goto("http://localhost:3000/check-offers")
        
        # -> Type 'Riley' into the first name input (index 1857) as the immediate action.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div[3]/div[2]/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Riley')
        
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div[3]/div[2]/div/div[2]/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Chen')
        ...
        # -> Type 'eighty thousand' into the Annual Income input (index 1892) as a non-numeric entry to trigger validation.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div[3]/div[2]/div[5]/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('eighty thousand')
        
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div[3]/div[2]/div[5]/div[2]/input').nth(0)
        await asyncio.sleep(3); await elem.fill('710')
