...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /payment-history and wait 15 seconds for the React SPA to mount, then verify the page title and required elements are visible.
        await page.goto("http://localhost:3000/payment-history")
        
        # --> Assertions to verify final state
        frame = context.pages[-1]
        current_title = await frame.evaluate("() => document.title")
        assert 'Payment' in current_title
